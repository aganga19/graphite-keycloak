...
public class FederatedJWTClientAuthenticator extends AbstractClientAuthenticator implements EnvironmentDependentProviderFactory {
    ...
    public void authenticateClient(ClientAuthenticationFlowContext context) {
        try {
            ...
            String idpAlias = context.getSession().identityProviders().getAllStream()
                    .filter(i -> issuer.equals(i.getConfig().get(IdentityProviderModel.ISSUER)))
                    .map(IdentityProviderModel::getAlias).findFirst().orElse(null);

            if (idpAlias == null) {
                return;
            }

            ClientAssertionIdentityProvider identityProvider = lookupIdentityProvider(context, idpAlias);
            ...
            ClientModel client = context.getSession().clients().getClientByAttributes(
                    context.getRealm(),
                    Map.of(FederatedJWTClientAuthenticator.PROVIDER_ID, idpAlias, FederatedJWTClientAuthenticator.JWT_CREDENTIAL_SUBJECT_KEY, federatedClientId));
    ...
    private ClientAssertionIdentityProvider lookupIdentityProvider(ClientAuthenticationFlowContext context, String idpAlias) {
