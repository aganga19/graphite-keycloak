...
public class NotifyUserStepProvider implements WorkflowStepProvider {
    ...
    private Map<ComponentModel, Long> getNextNonNotificationStep() {
        ...
        RealmModel realm = session.getContext().getRealm();
        ComponentModel policyModel = realm.getComponent(stepModel.getParentId());
        
        List<ComponentModel> steps = realm.getComponentsStream(policyModel.getId(), WorkflowStepProvider.class.getName())
            .sorted((a, b) -> {
