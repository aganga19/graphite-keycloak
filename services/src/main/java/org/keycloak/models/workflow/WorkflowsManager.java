...
public class WorkflowsManager {
    ...
    public void createSteps(Workflow workflow, List<Step> steps) {
        for (int i = 0; i < steps.size(); i++) {
            ...
            List<Step> subSteps = Optional.ofNullable(step.getSteps()).orElse(List.of());

            // persist the new step component.
            step = addStep(workflow.getId(), step);

            for (int j = 0; j < subSteps.size(); j++) {
                Step subStep = subSteps.get(j);
                // assign priority based on index.
    ...
    private Step addStep(String parentId, Step step) {
        RealmModel realm = getRealm();
        ...
        return new Step(realm.addComponentModel(stepModel));
    }
    ...
    public Stream<Step> getStepsStream(String parentId) {
        RealmModel realm = session.getContext().getRealm();
        return realm.getComponentsStream(parentId, WorkflowStepProvider.class.getName())
                .map(this::toStep).sorted();
    }

    private Step toStep(ComponentModel model) {
        Step step = new Step(model);
    ...
    public Step getStepById(KeycloakSession session, String id) {
        RealmModel realm = session.getContext().getRealm();
    ...
    private Step getFirstStep(Workflow workflow) {
        Step step = getSteps(workflow.getId()).get(0);
    ...
    public WorkflowStepProvider getStepProvider(Step step) {
                    ...
                    if (currentStep.getId().equals(scheduled.stepId())) {
                        ...
                        if (steps.size() > i + 1) {
                            ...
                            if (workflow.isRecurring()) {
                                Step firstStep = getFirstStep(workflow);
    ...
    public WorkflowRepresentation toRepresentation(Workflow workflow) {
        ...
        for (Step step : getSteps(workflow.getId())) {
    ...
    private WorkflowStepRepresentation toRepresentation(Step step) {
    ...
    private Step toModel(WorkflowStepRepresentation rep) {
        ...
        return new Step(rep.getProviderId(), rep.getConfig(), subSteps);
    }
